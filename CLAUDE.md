...
- **Step 2**: ✅ 已支持三元合成，后续可升级物理感知合成算法
- **合成模式**: 当前简单模式 → 未来物理模式（可配置切换）
...
- **适用性**: 适合快速验证和初期开发，但物理真实度有限

**未来升级方向**:
- **物理感知合成**: 考虑事件相机的物理特性（阈值、非线性响应等）
- **时空冲突处理**: 处理同一像素同一时间的多事件冲突
- **强度累积模型**: 模拟真实DVS传感器的光强累积和触发机制
- **噪声注入**: 添加符合DVS物理特性的传感器噪声模型
- **自适应融合**: 根据场景复杂度自动调整合成策略

**技术要点**:
- 保留当前简单合成作为baseline和快速模式
- 新增物理合成作为高精度模式，可配置切换
- 确保向后兼容性，不破坏现有工作流程
