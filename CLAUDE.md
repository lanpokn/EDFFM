...
- **Step 2**: ✅ 三元智能事件合成器 **[重构完成]**
  ...
  - **🆕时间处理**: 炫光+光源事件归一化后添加0-5ms随机偏移(模拟光源突然打开)
...
data:
  flare_synthesis:
    duration_range: [0.07, 0.095]               # 炫光事件时长(秒)，预留5ms给Step2随机偏移
    
# 🆕 时间线设计 (100ms固定总长度)
timing_design:
  step1_flare_timing: 0ms开始                 # Step1: 炫光从0开始
  step2_random_offset: [0, 5]ms               # Step2: 随机时间偏移(模拟光源突然打开)
  flare_duration: [70, 95]ms                   # 炫光持续时间，预留5ms给随机偏移  
...
- **Step1时间窗口**: 70-95ms随机长度，从0开始
- **Step2随机偏移**: 0-5ms随机时间偏移（模拟光源突然打开）
...
- **🆕 多样性增强**: 10%概率只使用散射炫光（scatter-only），90%概率使用散射+反射融合（scatter+reflection），增加数据集多样性 **[2025-08-29新增]**
- **状态**: ✅ 已集成到主流程，智能启用
...
# 时间线设计  
flare_duration: 70-95ms          # 炫光持续时间(预疕5ms给Step2随机偏移)
step1_start_offset: 0ms          # Step1: 固定从0开始
step2_random_offset: 0-5ms       # Step2: 随机时间偏移(模拟光源突然打开)
...
composition:
  ...
  physics_params:
    ...
    temporal_jitter_us: 10                  # 时间抖动范围 (微秒) - 基于DVS346真实jitter规格
