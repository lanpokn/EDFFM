...
## Model Architecture (Updated 2025-08-08)
- **Feature Extractor**: ⚠️ **TEMPORARILY 4D** (PFD disabled for performance) - [x_norm, y_norm, dt, p]
- **Mamba Backbone**: 4 layers, d_model=128, d_state=16
- **Classification**: Binary output for flare removal
- **Total Parameters**: Adjusted for 4D input (PFD restoration pending)

## 🎯 **当前焦点：特征提取器临时简化** (Updated 2025-08-08)

### **特征提取临时解决方案** (2025-08-08)
- **临时措施**: 禁用PFD特征提取，改为4维快速处理
- **4维特征**: [x_norm, y_norm, dt, polarity] - 归一化坐标 + 时间差 + 极性
- **处理方式**: 纯NumPy向量化操作，无循环，极速处理
- **用途**: 确保训练能够正常进行，后续恢复PFD特征
...
### ⚡ **特征提取器状态更新** (2025-08-08)
- **✅ 临时解决**: 禁用PFD，改为4维快速特征处理
- **🚨 之前的PFD问题** (已暂时搁置):
...
- **当前策略**: 
  1. ✅ **4维快速特征**: 向量化处理，秒级完成特征提取
  2. 🔄 **训练正常化**: 确保模型训练流程正常运行
  3. 📝 **PFD后续**: 在训练稳定后重新设计PFD特征
