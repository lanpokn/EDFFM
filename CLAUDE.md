...
### 📊 新输出格式 (标准DVS H5格式)
```bash
# Step 1 输出：散射+反射融合炫光事件 + 纯光源事件
output/data/flare_events/flare_sequence_xxx.h5         # 炫光事件(含GLSL反射)
├── /events/t  [N] int64    # 时间戳 (微秒)
├── /events/x  [N] uint16   # X坐标
├── /events/y  [N] uint16   # Y坐标  
└── /events/p  [N] int8     # 极性 (1/-1)

output/data/light_source_events/light_source_sequence_xxx.h5  # 纯光源事件(无反射)
├── /events/t  [N] int64    # 时间戳 (微秒，与炫光事件时间一致)
├── /events/x  [N] uint16   # X坐标
├── /events/y  [N] uint16   # Y坐标  
└── /events/p  [N] int8     # 极性 (1/-1)
...
# 分步执行  
python main.py --step 1 --debug  # 生成炫光事件和光源事件
python main.py --step 2 --debug  # 合成事件 (需要Step1先完成)

# 快速测试单个序列
python main.py --step 1 --debug  # 生成3个训练序列 + 3个验证序列
...
### 🚧 Step 1 扩展进行中：双事件生成 - 关键对齐问题待修复
**状态**: 已实现基础功能，但存在严重的位置对齐问题
- **✅ 已实现**: 同时生成光源事件 → `output/data/light_source_events/*.h5`
- **🔴 关键BUG**: **光源位置与炫光位置不对齐** - 最重要的问题！
- **✅ 数据源**: Flare7K数据集中的Light_Source图片文件夹 (5962张图片)
- **✅ 输出格式**: 同样的标准DVS格式 `/events/t,x,y,p`
- **✅ 文件命名**: 与炫光事件同名，如 `flare_sequence_xxx.h5` → `light_source_sequence_xxx.h5`
- **✅ 关键区别**: 光源事件**不包含GLSL反射炫光**，只有纯净的光源变化
- **✅ 错误处理**: 当光源变化太小无法产生DVS事件时，生成空事件数组而非报错

### 🚨 最重要的TODO：光源与炫光位置对齐
**问题**: 目前光源和炫光使用不同的随机变换，导致位置完全不匹配
**解决方案**: 
1. 炫光生成时保存所有变换参数（图片选择、位置变换、频闪参数等）
2. 光源生成时复用完全相同的参数，确保：
   - 选择对应的光源图片（同名文件）
   - 使用相同的位置变换参数
   - 使用相同的频闪和时间参数
3. 简洁实现：修改FlareFlickeringSynthesizer使其能接受固定参数而非随机生成
