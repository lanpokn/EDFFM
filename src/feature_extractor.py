...
class FeatureExtractor:
    ...
    def process_sequence(self, raw_events):
        ...
        output_dim = 11  # Reduced to 11 dimensions (removed cumulative features)
        ...
        for i in range(num_events):
            ...
            # Time delta features (handle overflow and negative values)
            if i > 0:
                dt = t - raw_events[i-1, 2]
                # Handle negative dt (from overflow) and zero dt
                dt = max(dt, 1)  # Ensure dt is at least 1 microsecond
                dt_norm = np.log(dt + 1e-6)  # Log-scaled delta time
                # Clamp to reasonable range
                dt_norm = np.clip(dt_norm, 0, 20)  # Prevent extremely large values
            else:
                dt_norm = 0
            ...
            dt_pixel_norm = np.log(max(dt_pixel, 1) + 1e-6) if dt_pixel > 0 else 0
            dt_pixel_norm = np.clip(dt_pixel_norm, 0, 20)  # Prevent extremely large values
            ...
            # Create optimized 11-dimensional feature vector (removed cumulative features)
            # Apply additional normalization/clipping to ensure stable ranges
            feature_vector = np.array([
                np.clip(x_center, -1, 1),            # 0: Center-relative x [-1,1]
                np.clip(y_center, -1, 1),            # 1: Center-relative y [-1,1]
                p,                                   # 2: Polarity {-1, 1}
                np.clip(dt_norm, 0, 15),             # 3: Log-scaled delta time [0,15]
                np.clip(dt_pixel_norm, 0, 15),       # 4: Log-scaled pixel-wise delta time [0,15]
                np.clip(mf_current, 0, 100),         # 5: Polarity frequency (Mf) [0,100]
                np.clip(ma_neighborhood, 0, 100),    # 6: Neighborhood polarity changes (Ma) [0,100]
                np.clip(ne_neighborhood, 0, 8),      # 7: Active neighbors count (Ne) [0,8]
